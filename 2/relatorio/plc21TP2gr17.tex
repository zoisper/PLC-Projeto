...
O problema consiste em implementar uma linguagem de programação imperativa simples, com regras sintáticas definidas pelo grupo. \\ \\ Para o desenvolvimento da nosso linguagem tivemos que definir uma gramática independente de contexto, \textbf{GIC}, e desenvolver um compilador que gera \textbf{pseudo-código} \verb Assembly \ para uma Máquina Virtual VM, que nos foi fornecida. \\ \\
...
\begin{itemize}
\item Definição da \textbf{GIC} que define a estrura sintática da nossa liguaguem.
...
Caso não haja erros sintáticos o \parser converte o código da nossa linguagem em codigo \verb Assembly \ da máquina virtual. Se houver erros, é mostrado ao utilizador uma mensagem de erro sintático. \\ \\
...
Na geração do código para obter o valor armazenado,por exemplo, numa variável do tipo inteiro optamos por realizar a seguinte operação:
\begin{verbatim}
PUSHGP 
...
\end{verbatim}
E para armazenar um valor inteiro numa variavel do tipo inteiro:
\begin{verbatim}
PUSHGP 
PUSHI <endereço da variável>
...
STOP
\end{verbatim}
Se o nosso acesso às variaveis utilizasse o processo mais simples, nao iriamos conseguir aceder a \verb arrays \ com um indice cujo valor está armazenado tambem numa variável. Assim, achamos que a nossa linguagem fica mais completa.\\ \\
Tambem se poderá verificar no anexo A, na geração de código, optamos por em cada produção envolvendo expressões númericas e variaveis, retornar sempre um tuplo com o código \verb Assembly \ da VM e um identificador do tipo de valor. Isto para nos premitir fazer conversão de valores de \verb int \ para \verb float \ e de \verb float \ para \verb int \ quando tal é necessário.
Por exemplo:
\begin{verbatim}
int a;
float b;
...
\section{Teste 11}
Erro de aceso a array com indice não inteiro. \\
...
Achamos que há aspetos que poderiam ser melhorados. Por exemplp, gostariamos que fosse possível atribuir o valor de uma condição a uma variável mas isso estava a gerar erros no \verb parser \ com rejeição de produções. \\ \\

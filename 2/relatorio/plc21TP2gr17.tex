...
\\ \\
\textbf{\underlina{Nota:}} na nossa linguagem não temos o tipo \verb booleano \ implementado. No seu lugar, tal como na linguagem \verb C, \ um valor superior a 0 é considerado \verb True \ e um valor inferior ou igual a 0 é considerado \verb False.  
...
Caso não haja erros sintáticos o \verb parser \ converte o código da nossa linguagem em código \verb Assembly \ da máquina virtual. Se houver erros, é mostrado ao utilizador uma mensagem de erro sintático. \\ \\
...
LOADN
\end{verbatim} 
Em vez de fazermos simplesmente:
\begin{verbatim}
...
STOP
\end{verbatim}
Se o nosso acesso às variáveis utilizasse o processo mais simples, nao iriamos conseguir aceder a \verb arrays \ com um índice cujo valor está armazenado também numa variável. Tomamos esta decisão, apesar de não ser pedido no enunciado do trabalho, pois assim a nossa linguagem fica mais completa.\\ \\
\textbf{\underlina{Nota 2:}}
Também se poderá verificar no anexo A, na geração de código, que optamos por em cada produção envolvendo expressões númericas e variáveis, retornar sempre um tuplo contendo o código \verb Assembly \ da VM e um identificador do tipo de valor. Isto permite-nos fazer conversão de valores de \verb int \ para \verb float \ e de \verb float \ para \verb int \ quando tal é necessário e com isso realizar operações entre variáveis de tipos diferentes.
Por exemplo:
\begin{verbatim}
int a;
float b;
...
O facto de produzirmos a nossa própria linguagem tornou a experiência bastante interessante, apesar de as regras gramaticais escolhidas a tornarem bastante parecida com a linguagem \verb C.  \\ \\
Achamos que há aspetos que poderiam ser melhorados. Por exemplo, gostariamos que fosse possível atribuir o valor de uma condição a uma variável mas isso estava a gerar alguns conflitos no \verb parser \ com a consequente rejeição de produções. \\ \\
Também gostariamos que nossa linguagem fosse possível colocar parênteses entre condições lógicas por forma a alterar a precedencia dos operadores \verb or \ e \verb and. \ Tal seria possível, no anexo A do documento temos a seguinte produção em comentário: 
\begin{verbatim}
def p_preposition_condition_between_parenthesis(p):
 ...
 p[0] = p[2]
\end{verbatim}
Bastaria 'descomentar' esta produção para que tal fosse possível. No entanto se o fizessemos, ao executarmos o \verb parser, \ iriamos gerar um conflito, muito provavelmente devido a outra produção que temos:
\begin{verbatim}
def p_factor_expression_between_parenthesis(p):
  ...
  p[0] = p[2]
\end{verbatim}
Como não queremos que haja conflitos entre poduções, optamos por deixar a produção em comentário.  \\ \\
...
\textbf{Ficheiro de teste quadrado.txt}
\begin{scriptsize}
\begin{verbatim}
main
{
...
\textbf{Ficheiro de teste menor.txt}
\begin{scriptsize}
\begin{verbatim}
main
{
...
\textbf{Ficheiro de teste produto.txt}
\begin{scriptsize}
\begin{verbatim}
main
{
...
\textbf{Ficheiro de teste ordena.txt}
\begin{scriptsize}
\begin{verbatim}
main
{
...
\textbf{Ficheiro de teste transposta.txt}
\begin{scriptsize}
\begin{verbatim}
main
{

...
Caso não haja erros sintáticos o \parser converte o código da nossa linguagem em codigo \verb Assembly \ da máquina virtual. Se houver erros, é mostrado ao utilizador uma mensagem de erro sitático. \\ \\
...
Achamos que há aspetos que poderiam ser melhorados, como por exemplo a implementação de funções com e sem argumentos. Também na parte de atribuição de valores às variaveis há uma pequena 'falha' que permite ao utilizador por exemplo fazer:
\begin{verbatim}
a = 1,;
\end{verbatim}
No entanto se esta 'falha' fosse corrigida, teriamos, por exemplo, que abdicar da seguinte instrução:
\begin{verbatim}
a = 1,b = 2;
\end{verbatim}
Tendo a instrução que ficar obrigatoriamente do género:
\begin{verbatim}
a = 1;
b = 2;
\end{verbatim}
Também os ciclos \textit{for} teriam que ser escirtos da seguinte forma:
\begin{verbatim}
for(i=0;i<N;i=i+i;){}
\end{verbatim}
